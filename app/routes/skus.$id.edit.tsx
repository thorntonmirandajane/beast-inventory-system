...
export const loader = async ({ request, params }: LoaderFunctionArgs) => {
  ...
  const allSkus = await prisma.sku.findMany({
    where: { isActive: true, id: { not: id } }, // Exclude current SKU
  ...
  return { user, sku, allSkus, activeProcesses, categories };
};
...
export const action = async ({ request, params }: ActionFunctionArgs) => {
  ...
  const skuCode = (formData.get("sku") as string)?.trim().toUpperCase();
  ...
  // Check if SKU code already exists (excluding current SKU)
  const existing = await prisma.sku.findUnique({ where: { sku: skuCode } });
  if (existing && existing.id !== id) {
    return { error: `SKU "${skuCode}" ALREADY EXISTS` };
  }
...
export default function EditSku() {
  const { user, sku, allSkus, activeProcesses, categories } = useLoaderData<typeof loader>();
  const actionData = useActionData<typeof action>();
  const navigation = useNavigation();
  const isSubmitting = navigation.state === "submitting";

  const [selectedType, setSelectedType] = useState<string>(sku.type);

  // Initialize selected components from existing BOM
  const initialComponents: SelectedComponent[] = sku.bomComponents.map(bc => ({
    skuId: bc.componentSku.id,
    sku: bc.componentSku.sku,
    name: bc.componentSku.name,
    type: bc.componentSku.type,
  ...
  const addComponent = (skuToAdd: typeof allSkus[0]) => {
  ...
  return (
    <Layout user={user}>
      <div className="mb-6">
        <Link to={`/skus/${sku.id}`} className="text-sm text-gray-500 hover:text-gray-700">
          ← BACK TO SKU DETAILS
        </Link>
      </div>

      <div className="page-header">
        <h1 className="page-title">EDIT SKU</h1>
        <p className="page-subtitle">UPDATE PRODUCT OR COMPONENT INFORMATION</p>
      </div>
      ...
      <Form method="post">
              ...
              {selectedType === "COMPLETED" && (
                <div className="form-group md:col-span-2">
                  ...
                  <input
                    ...
                    placeholder="UPC barcode for completed products"
                  />
                  <p className="text-sm text-gray-500 mt-1">
                    UPC will be used for barcode printing instead of SKU code
                  </p>
                </div>
              )}
              <div className="form-group md:col-span-2">
            ...
            <div className="card-body">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 max-h-64 overflow-y-auto">
                  {availableComponents.map((skuItem) => (
                    <button
                      key={skuItem.id}
                      type="button"
                      onClick={() => addComponent(skuItem)}
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        <div className="font-mono text-sm truncate">
                          {skuItem.sku.toUpperCase()}
                        </div>
                        <div className="text-xs text-gray-500 truncate">
                          {skuItem.name.toUpperCase()}
                        </div>
                      </div>
                      <svg
        ...
        <div className="flex gap-3">
          ...
          <Link to={`/skus/${sku.id}`} className="btn btn-secondary">
            CANCEL
          </Link>
        </div>
      </Form>
